...
function HomeContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const eRichiestaAssenza = 
        categoriaForm === 'ferie' || 
        categoriaForm === 'permesso' || 
        testoProgetto.includes('ferie') || 
        testoProgetto.includes('permesso') || 
        testoProgetto.includes('rol') || 
        testoCliente.includes('assenze');
      ...
      for (const d of dateDaSalvare) {
        ...
        if (res.ok) {
          ...
          ultimoMessaggioErrore = errData.message || 'Errore durante il salvataggio nel database.';
        }
      }
      ...
      if (salvatiOk > 0) {
        ...
        setStatusMessage({ type: 'error', text: ultimoMessaggioErrore || 'Errore durante il salvataggio.' });
      }
    } catch (err) {
      setStatusMessage({ type: 'error', text: 'Errore di comunicazione con il server.' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="flex-1 p-4 md:p-8 w-full max-w-5xl mx-auto space-y-6">
        ...
        {activeTab === 'home' && (
          <div className="space-y-8">
            {/* BANNER NOTIFICA ADMIN NUOVE FERIE IN ATTESA */}
            {currentUser?.ruolo === 'admin' && assenzeDaApprovareAdmin.length > 0 && (
              <div className="bg-gradient-to-r from-purple-700 to-indigo-800 text-white p-4 rounded-3xl shadow-xl border-2 border-purple-400 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 bg-amber-400 text-slate-950 font-black rounded-2xl flex items-center justify-center text-xl shadow-md">
                    🚨
                  </div>
                  <div>
        ...
        {/* TAB 2: GESTIONE ATTIVITÀ CON APPROVAZIONE IN EVIDENZA */}
        {activeTab === 'programmati' && (
          <div className="space-y-6">
            ...
            {/* BOX APPROVAZIONE RAPIDA FERIE ADMIN */}
            {currentUser?.ruolo === 'admin' && assenzeDaApprovareAdmin.length > 0 && (
              <div className="bg-gradient-to-r from-purple-900 to-slate-900 text-white rounded-3xl p-6 shadow-xl border border-purple-800 space-y-4">
                <div className="flex items-center justify-between border-b border-purple-800/80 pb-3">
                  <h3 className="font-bold text-base flex items-center gap-2">
                    <span>🏖️</span> Richieste Ferie e Permessi da Approvare ({assenzeDaApprovareAdmin.length})
                  </h3>
                  ...
                  {assenzeDaApprovareAdmin.map((item, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2" onClick={e => e.stopPropagation()}>
                        ...
                        <button 
                          onClick={() => handleRifiutaAssenza(item)} 
                          className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white font-extrabold text-xs rounded-xl shadow-md transition-all whitespace-nowrap"
                        >
                          ❌ Rifiuta
                        </button>
                      </div>
                    ...
                    >
                      ...
                      {!isDir && (
                        <div className="flex items-center space-x-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                          <button 
                            type="button"
                            onClick={() => setModalDocumento(item)}
        ...
        {/* TAB 5: CENTRO REPORTISTICA E ARCHIVI */}
        {activeTab === 'cruscotto' && currentUser?.ruolo === 'admin' && (
          <div className="space-y-6">
            ...
            {/* SUB-TAB 3: ARCHIVIO FERIE E PERMESSI */}
            {subTabReport === 'ferie' && (
              <div className="bg-white rounded-3xl shadow-lg border border-slate-200/80 p-6 space-y-6">
                <div className="border-b border-slate-100 pb-3">
                  <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                    <span>🏖️</span> Archivio Ferie &amp; Permessi (Per Dipendente)
                  </h3>
                  <p className="text-xs text-slate-500">
                    Riepilogo delle richieste in attesa (sempre visibili) e delle assenze approvate filtrate per il mese di <strong>{filtroMeseReport}</strong>.
                  </p>
                </div>
                ...
                <div className="space-y-5">
                  {listaDipendenti.map(nomeDip => {
                    ...
                    return (
                      <div key={nomeDip} className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                        <div className="bg-slate-50 p-3.5 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
                          ...
                          <div className="flex gap-2">
                            {inApprovazione.length > 0 && <span className="text-[10px] font-bold bg-amber-100 text-amber-800 px-2 py-1 rounded-lg">⏳ {inApprovazione.length} Da Approvare</span>}
                            {approvate.length > 0 && <span className="text-[10px] font-bold bg-emerald-100 text-emerald-800 px-2 py-1 rounded-lg">✅ {approvate.length} Approvate ({filtroMeseReport})</span>}
                          </div>
                        </div>
                        ...
                        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                          {/* IN APPROVAZIONE */}
                          <div className="space-y-3">
                            <h4 className="text-xs font-black text-amber-700 uppercase tracking-wider border-b border-amber-100 pb-1.5 flex items-center gap-1"><span>⏳</span> In Approvazione</h4>
                          ...
                          {/* APPROVATE */}
                          <div className="space-y-3">
                            <h4 className="text-xs font-black text-emerald-700 uppercase tracking-wider border-b border-emerald-100 pb-1.5 flex items-center gap-1"><span>✅</span> Approvate ({filtroMeseReport})</h4>
                            {approvate.length === 0 ? (
                  ...
                  {listaDipendenti.every(nomeDip => {
                    ...
                    return inApprovazione.length === 0 && approvate.length === 0;
                  }) && (
                    <div className="text-center py-10 bg-slate-50 border border-dashed rounded-2xl text-xs text-slate-400">
                      <span className="text-3xl block mb-2">🏖️</span>
                      Nessuna ferie o permesso presente per il mese di {filtroMeseReport}, né richieste in attesa.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-slate-900 text-white p-4 flex items-center justify-between gap-3 border-b border-slate-800">
                ...
                <div className="flex items-center space-x-2 flex-shrink-0">
                  ...
                  <button 
                    ...
                    className="p-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl font-bold transition-all text-sm cursor-pointer"
                  >
                    ✕
                  </button>
                </div>
